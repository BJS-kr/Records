...
class PixabaySearchVideosOptions {
  lang?:string;
  id?:string;
  video_type?:string; 
  category?:string;
...
export class PixabayVideos implements PixabaySearchVideos {
  ...
  public async search(q:string, searchOptions?:PixabaySearchVideosOptions) {
    ...
    if ('editors_choice' in searchOptions) {requestingURL+=`&editors_choice=${searchOptions.editors_choice}`}
    if ('safesearch' in searchOptions) {requestingURL+=`&safesearch=${searchOptions.safesearch}`}
